...
my.matrix[,1] <- seq(1,10)
my.matrix[,2] <- seq(1,10)*2
my.matrix[,3] <- seq(1,10)*3

for (column in 1:5) { #outer
        for (row in 1:10) { # inner
                my.matrix[row,column] <- row*column
        }
}
...
numbersfromcolors <- rep(NA, length(colors2numbers))
for(k in 1:length(colors2numbers)){
        numbersfromcolors[k] <- colortest(colors2numbers[k])
}
